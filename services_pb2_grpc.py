# Generated by the gRPC Python protocol compiler plugin. DO NOT EDIT!
"""Client and server classes corresponding to protobuf-defined services."""
import grpc

import services_pb2 as services__pb2


class Service1Stub(object):
    """Missing associated documentation comment in .proto file."""

    def __init__(self, channel):
        """Constructor.

        Args:
            channel: A grpc.Channel.
        """
        self.ListenAndSend = channel.unary_unary(
                '/Service1/ListenAndSend',
                request_serializer=services__pb2.Request.SerializeToString,
                response_deserializer=services__pb2.Response.FromString,
                )


class Service1Servicer(object):
    """Missing associated documentation comment in .proto file."""

    def ListenAndSend(self, request, context):
        """Missing associated documentation comment in .proto file."""
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')


def add_Service1Servicer_to_server(servicer, server):
    rpc_method_handlers = {
            'ListenAndSend': grpc.unary_unary_rpc_method_handler(
                    servicer.ListenAndSend,
                    request_deserializer=services__pb2.Request.FromString,
                    response_serializer=services__pb2.Response.SerializeToString,
            ),
    }
    generic_handler = grpc.method_handlers_generic_handler(
            'Service1', rpc_method_handlers)
    server.add_generic_rpc_handlers((generic_handler,))


 # This class is part of an EXPERIMENTAL API.
class Service1(object):
    """Missing associated documentation comment in .proto file."""

    @staticmethod
    def ListenAndSend(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_unary(request, target, '/Service1/ListenAndSend',
            services__pb2.Request.SerializeToString,
            services__pb2.Response.FromString,
            options, channel_credentials,
            insecure, call_credentials, compression, wait_for_ready, timeout, metadata)


class Service2Stub(object):
    """Missing associated documentation comment in .proto file."""

    def __init__(self, channel):
        """Constructor.

        Args:
            channel: A grpc.Channel.
        """
        self.ExecuteRoutine = channel.unary_unary(
                '/Service2/ExecuteRoutine',
                request_serializer=services__pb2.Request.SerializeToString,
                response_deserializer=services__pb2.Response.FromString,
                )


class Service2Servicer(object):
    """Missing associated documentation comment in .proto file."""

    def ExecuteRoutine(self, request, context):
        """Missing associated documentation comment in .proto file."""
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')


def add_Service2Servicer_to_server(servicer, server):
    rpc_method_handlers = {
            'ExecuteRoutine': grpc.unary_unary_rpc_method_handler(
                    servicer.ExecuteRoutine,
                    request_deserializer=services__pb2.Request.FromString,
                    response_serializer=services__pb2.Response.SerializeToString,
            ),
    }
    generic_handler = grpc.method_handlers_generic_handler(
            'Service2', rpc_method_handlers)
    server.add_generic_rpc_handlers((generic_handler,))


 # This class is part of an EXPERIMENTAL API.
class Service2(object):
    """Missing associated documentation comment in .proto file."""

    @staticmethod
    def ExecuteRoutine(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_unary(request, target, '/Service2/ExecuteRoutine',
            services__pb2.Request.SerializeToString,
            services__pb2.Response.FromString,
            options, channel_credentials,
            insecure, call_credentials, compression, wait_for_ready, timeout, metadata)
